...
router.route('^/$|/index(.html)?')     // handles all requests made by the client for the index.html page
...
router.route('/search(.html)?')    // handles all requests to search.html from clients
...
router.route('/upload(.html)?')    // handles all requests to the upload.html page from client
...
router.route('/login(.html)?') // handles all requests to login.html
    .get((request, response) => {   // get requests handled here
        console.log(`${request.method}\t${request.headers.origin}\t${request.url}`);    // log request details
        if (typeof(request.session.user) !== "undefined" && request.session.user) { // if a user is logged in
...
router.route('/registration(.html)?')  // handles all requests to registration.html
    .get((request, response) => {   // get requests handled here
        console.log(`${request.method}\t${request.headers.origin}\t${request.url}`);    // log request details
...
router.route('/liked(.html)?') // handles all requests to liked.html
    .get((request, response) => {   // get requests handled here
        console.log(`${request.method}\t${request.headers.origin}\t${request.url}`);    // log request details
        if (typeof(request.session.user) !== "undefined" && request.session.user) { // if a user is logged in
