...
USER root
RUN apt-get update && \
    apt-get install -y curl gnupg2 lsb-release build-essential g++ gcc git software-properties-common apt-transport-https ca-certificates  && \
    apt-get install -y nodejs npm mariadb-server-core mariadb-client-core mariadb-common nano && \
    apt-get upgrade
